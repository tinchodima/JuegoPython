...
def clasificarPalabraWiktionary(palabra):
    ...
    try:
        ...
        if clasificacion.lower() == "adjetivo" or clasificacion.lower() == "forma adjetiva":
            encontre = 'JJ'
            return encontre

        if clasificacion.lower() == "verbo" or clasificacion.lower() == "verbo intransitivo":
            encontre = 'VB'
            return encontre
...
def comprobarWikPattern2(palabra, listaPalabras, values):
    ...
    if clasificarPalabraWiktionary(palabra) == '9999' and clasificarPalabraPattern(palabra) != '9999':
        ...
        sg.Popup(
            'La clasificacion no se encuentra en Wiktionary, ingresar una definicion manualmente')
...
def obtenerDefinicion(palabra):
    ...
    a = w.search(palabra)
    try:
        definicion = a.sections[3].content.split(
            '1')[1].split('.2')[0].split('*')[0]
    except(IndexError):
        try:
            definicion = a.sections[3].content.split('1')[0].split('*')[1]
            if ' ' in definicion[1]:
                try:
                    definicion = a.sections[3].content.split('1')[
                        0].split('*')[2]
                except(IndexError):
                    definicion = a.sections[3].content.split('1')[
                        0].split('*')[3]
...
def promedio():
    dire = os.path.abspath(os.path.join(os.path.join(
        os.pardir, 'Raspberry'), 'datos-oficinas.json'))
    ...
    try:
        ...
        sg.PopupNoButtons('No se encontraron oficinas', title='',
                          auto_close_duration=1, auto_close=True)

    return promedioTemperaturas


layout = [
    [sg.T('DIGOM: SOPA DE LETRAS', size=(32, 1), font=(
        'Time New Roman', 20), background_color='#80cbc4')],
    [sg.T('● Cantidad de palabras que se van a ingresar a la sopa de letras',
          text_color='black', font=('Time New Roman', 12), background_color='#80cbc4')],
    [sg.T('Sustantivos', text_color='black', font=('Time New Roman', 10), background_color='#80cbc4'), sg.Spin([i for i in range(0, 11)], initial_value=0, size=(2, 2), key=('cantS')), sg.Text('Adjetivos', text_color='black', font=('Time New Roman', 10), background_color='#80cbc4'),
     sg.Spin([i for i in range(0, 11)], initial_value=0, size=(2, 2), key=('cantA')), sg.Text('Verbos', text_color='black', font=('Time New Roman', 10), background_color='#80cbc4'), sg.Spin([i for i in range(0, 11)], initial_value=0, size=(2, 2), key=('cantV'))],
    [sg.T('● Ingrese una palabra:', text_color='black', font=('Time New Roman', 12), background_color='#80cbc4'),
     sg.InputText(key='textoIngresado'), sg.Submit('Agregar'), sg.Submit('Quitar')],
    [sg.Multiline(key='dato', size=(70, 1), font='Arial', text_color='blue')],
    [sg.T('● Nivel de dificultad:   ', text_color='black', font=('Time New Roman', 12), background_color='#80cbc4'), sg.Radio('Sin ayuda ', "RADIO1", default=True, background_color='#80cbc4',
                                                                                                                              key='sinAyuda'), sg.Radio('Con definiciones', "RADIO1", background_color='#80cbc4', key='mosDef'), sg.Radio('Mostrando las palabras a buscar', "RADIO1", background_color='#80cbc4', key='mosPal')],
    [sg.T('● Orientación de las palabras:   ', text_color='black', font=('Time New Roman', 12), background_color='#80cbc4'), sg.Radio('Horizontal',
                                                                                                                                      "RADIO2", default=True, background_color='#80cbc4', key='horizontal'), sg.Radio('Vertical', "RADIO2", background_color='#80cbc4', key='vertical')],
    [sg.T('● Fuente: ', text_color='black', font=('Time New Roman', 12), background_color='#80cbc4'), sg.InputCombo(
        ['Arial', 'Helvetica', 'Calibri', 'Consolas', 'Tahoma', 'Courier', 'Verdana', 'Times', 'Fixedsys'], size=(12, 20), key='font', readonly=True)],
    [sg.T('● Tipo: ', text_color='black', font=('Time New Roman', 12), background_color='#80cbc4'),
     sg.InputCombo(['Mayúscula', 'Minúscula'], size=(12, 20), key='mayOmin', readonly=True)],
    [sg.T('● Oficina', background_color='#80cbc4', font=('Time New Roman', 12)),
     sg.InputCombo(values=list(promedio().keys()), key='oficinas', readonly=True)],
    [sg.T('● Elegir colores: ', text_color='black', font=('Time New Roman', 12), background_color='#80cbc4'), sg.ColorChooserButton('Sustantivos', button_color=(
        '#000000', '#03A9F4')), sg.ColorChooserButton('Adjetivos', button_color=('#000000', '#03A9F4')), sg.ColorChooserButton('Verbos', button_color=('#000000', '#03A9F4'))],
    [sg.Submit('Generar sopa de letras'), sg.Cancel('Salir')]
]
window = sg.Window('Seminario de Lenguajes 2019: Python', font=(
    'Arial', 10), background_color='#80cbc4').Layout(layout)
...
while True:
    ...
    if button == 'Generar sopa de letras':
        if len(listaPalabras) == 0:
            ...
            sg.Popup(
                '¡Debes darle un valor a la cantidad de palabras que vas a elegir, sino la sopa de letras va a salir vacia!')
        else:
            break
...
if button == 'Salir':
    ...
    if values['Sustantivos'] == '':
        colorS = 'yellow'
        sg.Popup('Como no se agrego un color específico a los SUSTANTIVOS tendrá un color por defecto: AMARILLO', text_color='yellow')
    else:
    ...
    if values['Adjetivos'] == '':
        colorA = 'red'
        sg.Popup(
            'Como no se agrego un color específico a los ADJETIVOS tendrá un color por defecto: ROJO', text_color='red')
    else:
    ...
    if values['Verbos'] == '':
        colorV = 'green'
        sg.Popup(
            'Como no se agrego un color específico a los VERBOS tendrá un color por defecto: VERDE', text_color='green')
    else:
