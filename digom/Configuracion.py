...
class Configuracion:
    ...
    def clasificarPalabraPattern(self,palabra):
        '''
    ...
    def comprobarWikPattern(self, palabra,__listaPalabras,values):
        if self.clasificarPalabraPattern(palabra) == self.clasificarPalabraWiktionary(palabra) and self.clasificarPalabraPattern(palabra) != 'No se pudo clasificar':
            self.clasificacionWiktionary(palabra,__listaPalabras,values)
            self.__definiciones[palabra] = self.obtenerDefinicion(palabra)
            print('wik1')
        elif self.clasificarPalabraWiktionary(palabra) == 'No se pudo clasificar' and self.clasificarPalabraPattern(palabra) == 'No se pudo clasificar':
            ...
            print('ninguno')
        elif self.clasificarPalabraWiktionary(palabra) != self.clasificarPalabraPattern(palabra) and self.clasificarPalabraWiktionary(palabra) != 'No se pudo clasificar':
            self.clasificacionPattern(palabra,__listaPalabras)
            print('pattern1')
            ...
            self.__definiciones[palabra] = text
            #pedir que ingrese una definicion
        elif self.clasificarPalabraWiktionary(palabra) != self.clasificarPalabraPattern(palabra) and self.clasificarPalabraWiktionary(palabra) != 'No se pudo clasificar' and self.clasificarPalabraPattern(palabra) != 'No se pudo clasificar':
            self.clasificacionPattern(palabra,__listaPalabras)
            print('pattern2')
            sg.Popup('La clasificacion no se encuentra en Wiktionary, ingresar una definicion manualmente')
            text = sg.PopupGetText(palabra, 'Ingrese una definicion')
            self.__definiciones[palabra] = text
            #pedir que ingrese una definicion
        elif self.clasificarPalabraPattern(palabra) != self.clasificarPalabraWiktionary(palabra) and self.clasificarPalabraPattern(palabra) != 'No se pudo clasificar':
            ...
            self.reporteClasificaciones(msg)
            print('wik2')
        elif self.clasificarPalabraPattern(palabra) != self.clasificarPalabraWiktionary(palabra) and self.clasificarPalabraPattern(palabra) != 'No se pudo clasificar' and self.clasificarPalabraWiktionary(palabra) != 'No se pudo clasificar':
            #informar en un reporte que la clasificacion no existe en pattern
            self.clasificacionWiktionary(palabra,__listaPalabras,values)
            self.__definiciones[palabra] = self.obtenerDefinicion(palabra)
            msg = 'Clasificacion no encontrada en Pattern'
    ...
    def clasificacionWiktionary(self,palabra,__listaPalabras,values):
        try:
            if self.comprobarQueLaPalabraNoEsteAgregada(palabra,__listaPalabras) == False:
                sg.Popup('La palabra ya esta agregada en la sopa de letras')
            else:
                if self.clasificarPalabraWiktionary(palabra) == 'NN':
                    if str(len(self.__listaSustantivos)) < values['cantS']:
                        self.__listaSustantivos.append(palabra)
                        self.__listaPalabrasAceptadas.append(palabra)
                    else:
                        sg.Popup('El limite de Sustantivos se alcanzo')
                elif self.clasificarPalabraWiktionary(palabra) == 'JJ':
                    if str(len(self.__listaSustantivos)) < values['cantA']:
                        self.__listaPalabrasAceptadas.append(palabra)
                        self.__listaSustantivos.append(palabra)
                    else:
                        sg.Popup('El limite de Adjetivos se alcanzo')
                elif self.clasificarPalabraWiktionary(palabra) == 'VB':
    ...
    def clasificacionPattern(self,palabra,__listaPalabras):
        try:
            if self.comprobarQueLaPalabraNoEsteAgregada(palabra,__listaPalabras) == False:
                sg.Popup('La palabra ya esta agregada en la sopa de letras')
            else:
                if self.clasificarPalabraPattern(palabra) == 'NN':
                    self.__listaSustantivos.append(palabra)
                    self.__listaPalabrasAceptadas.append(palabra)
                elif self.clasificarPalabraPattern(palabra) == 'JJ':
                    self.__listaAdjetivos.append(palabra)
                    self.__listaPalabrasAceptadas.append(palabra)
                elif self.clasificarPalabraPattern(palabra) == 'VB':
                    self.__listaVerbos.append(palabra)
                    self.__listaPalabrasAceptadas.append(palabra)
                else:
                    print('No se pudo agregar')
    ...
    def comprobarQueLaPalabraNoEsteAgregada(self,palabra,__listaPalabras):
        if palabra in self.__listaPalabras:
            return False
        else:
            return True
